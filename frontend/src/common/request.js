...
function request(con) {
	...
	let config = { //判断是否有以下属性没有就赋予默认值
		...
		complete: con.complete ? (res) => {
			...
			if (!res.statusCode) {
				// 把底层失败原因一起显示出来，便于在真机上定位是超时、DNS 还是被拦截
				var why = String(res.errMsg || '').replace('request:fail ', '').trim();
				console.log('[request fail]', res.errMsg, con.url);
				uni.showToast({
					title: why ? ('请求失败 · ' + why).slice(0, 40) : '请求失败',
					icon: 'none',
					duration: 4000
				});
				uni.hideLoading();
...
// 	}
// });
// 地址及配置（本地部署：统一在 @/common/config.js 中修改）
http.setBaseUrl(config.API_HOST);
export default http
