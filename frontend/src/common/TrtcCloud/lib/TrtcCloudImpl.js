...
export default class TrtcCloudImpl {
    ...
    setRemoteAudioVolume(userId, volume) {
        if (typeof userId !== NAME.STRING || typeof volume !== NAME.NUMBER) {
            throw new TrtcError({
                code: TXLiteJSError.INVALID_PARAMETER,
                message: `${NAME.LOG_PREFIX} please check the setRemoteAudioVolume method parameters. userId type is a ${typeof userId}, volume type is a ${typeof volume} not a ${NAME.NUMBER}`,
            });
        }
        try {
            TrtcNativeTrtcCloudModule.setRemoteAudioVolume({ userId, volume });
        }
        catch (error) {
            throw generateError_(error);
        }
