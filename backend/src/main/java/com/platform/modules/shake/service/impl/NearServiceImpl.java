...
public class NearServiceImpl implements NearService {
    ...
    private List<NearVo02> getNear() {
        // 当前用户
        String userId = NumberUtil.toStr(ShiroUtils.getUserId());
        // 半径来自配置 platform.nearRadius（默认 100 公里），最多 99 人
        List<GeoResult<GeoVo>> geoResults = geoHashUtils.radius(AppConstants.REDIS_NEAR, userId,
                PlatformConfig.NEAR_RADIUS, 99);
        // 过滤
