...
if (-not (Test-Path $ks)) {
    $r = Invoke-Native "$JDK\bin\keytool.exe" @('-genkeypair','-keystore',$ks,'-alias','msm','-keyalg','RSA','-keysize','2048',
        '-validity','10000','-storepass','<REDACTED_KEYSTORE_PW>','-keypass','<REDACTED_KEYSTORE_PW>',
        '-dname','CN=Msm, OU=Dev, O=Msm, L=Lincang, ST=Yunnan, C=CN')
    if ($r.Code -ne 0) { Show-Fail $r "keytool" }
}
$r = Invoke-Native "$BT\apksigner.bat" @('sign','--ks',$ks,'--ks-pass','pass:<REDACTED_KEYSTORE_PW>','--key-pass','pass:<REDACTED_KEYSTORE_PW>',
